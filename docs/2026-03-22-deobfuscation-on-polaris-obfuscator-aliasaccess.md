...
实现代码见 `alias.py` + `utils/vex_utils.py`，整体流程分为三个阶段：**定位** → **求解** → **Patch**。下面逐步展开。
...
如果 getter 被 MBA（Mixed Boolean-Arithmetic）混淆，VEX pattern match 会失败。此时自动回退到 **per-getter symex**——对这单个小函数做符号执行，输入 symbolic `rdi`，求解 `rax - rdi` 得到 offset：
...
最小粒度——只跑一个 3 指令的函数，零路径爆炸风险。即使 `add` 被改写成一堆 `xor/and/or`，symex 照样能求解。
...
> 早期方案使用 DDG（数据依赖图）的后向切片来判断 use site 是否与 getter 函数有关，但 DDG 存在精度问题（寄存器别名导致误报），且只能检测 VEX `Store` 语句，漏掉了所有的读操作。最终方案完全基于 CFG 结构，不需要 DDG，也不需要 CFGEmulated——`CFGFast` 就够了。
...
早期实现使用 `simgr.explore(find=use_block_addr)` 从函数入口跑到目标块。这有三个根本问题：

1. **SkipFunc 可达性**：函数体中有 `crc32_gentab`、`func_1` 等大函数，必须 hook 跳过。但如果某个 use site 被 hook 函数的返回值 guard 住（`if(func_1() == X)`），hook 返回 0 可能导致 symex 走错分支，永远到不了那个块。
2. **路径爆炸**：即使用 `ZERO_FILL` 压制符号分支，也可能因为条件码跑偏而找不到目标。回退到 `SYMBOL_FILL` 又会分叉。
3. **粒度太粗**：getter 链的解析**只依赖 prologue 初始化的 transit node 内存和各 getter 的常量 offset**。根本不需要执行函数体中间的任何代码。

#### Chain-walk：分层求解

最终方案完全不跑全函数 symex。符号执行只用在两个最小粒度的地方：

- **Prologue**（一次性）：执行函数序言块来初始化 transit node 内存
...
**① 后向遍历 getter 链**。从使用块出发，沿 CFG 向后走，每一步找到以 getter call 结尾的前驱块，记录 getter offset。当遇到**链起点**时停止——链起点的判定条件有两个：

- **Self-deref 边界**（主要）：如果当前块自身包含 self-deref，说明它是上一条链的 use site，同时也是本条链的起点（AliasAccess 的块结构是交错的）。
- **非 getter 前驱**（兜底）：如果当前块的前驱不是 getter call（比如 prologue），则它就是链的第一跳。
...
**② 获取起始指针**。链入口块包含一条 `lea rdi, [rbp-K]`（加载 transit node 地址）和紧接着的 `call getter`。我们不解析 `lea` 指令的编码，而是对这个**单块做一次 symex**——用 prologue 状态的寄存器和内存执行这一个块，然后读取 `rdi` 的具体值：
...
**③ 前向读取 transit node 内存**。拿到起始指针后，沿链的每一跳：`ptr = prologue_mem[ptr + getter_offset]`。最后一次读取得到的就是 raw struct 的地址。
